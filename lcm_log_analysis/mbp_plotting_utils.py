...
sys.path.insert(0, "/home/xuanhien/git/dairlib/bazel-bin/bindings/pydairlib/multibody")
...
from osc_debug import (
    lcmt_osc_tracking_data_t,
    osc_tracking_cost,
    osc_regularlization_tracking_cost,
)
...
#     CreateActuatorNameVectorFromMap,
# )
...
def process_state_channel(state_data, plant):
    ...
    return {"t": np.array(t), "q": np.array(q), "v": np.array(v), "u": np.array(u)}
...
def make_point_positions_from_q(
    ...
    for i, generalized_pos in enumerate(q):
        ...
        pos[i] = plant.CalcPointsPositions(
            context, frame, pt_on_frame, frame_to_calc_position_in
        ).ravel()
...
def get_floating_base_velocity_in_body_frame(robot_output, plant, context, fb_frame):
    ...
    for i, (q, v) in enumerate(zip(robot_output["q"], robot_output["v"])):
        ...
        vel[i] = fb_frame.CalcSpatialVelocity(
            context, plant.world_frame(), fb_frame
        ).translational()
...
def process_osc_channel(data):
    ...
    for msg in data:
        ...
        if hasattr(msg, "regularization_cost_names"):
            regularization_costs.append(
                msg.regularization_cost_names, msg.regularization_costs
            )
        else:
            regularization_cost_names = [
                "input_cost",
                "acceleration_cost",
            ...
            regularization_cost_list = [
                msg.input_cost,
                msg.acceleration_cost,
                msg.soft_constraint_cost,
            ]
            regularization_costs.append(
                regularization_cost_names, regularization_cost_list
            )
        ...
        for tracking_data in msg.tracking_data:
            if tracking_data.name not in osc_debug_tracking_datas:
                osc_debug_tracking_datas[tracking_data.name] = lcmt_osc_tracking_data_t(
...
def load_default_channels(data, plant, state_channel, input_channel, osc_debug_channel):
    ...
    # osc_debug = permute_osc_joint_ordering(osc_debug, data[state_channel][0], plant)
    # osc_debug = None
    robot_output = None
    robot_input = None
...
def plot_positions_by_name(
    robot_output, q_names, time_slice, pos_map, ps=None, subplot_index=0
):
...
def plot_measured_efforts_by_name(
    robot_output, u_names, time_slice, u_map, ps=None, subplot_index=0
):
...
def plot_points_positions(
    ...
    for name in frame_names:
        ...
        pt = pts[name]
        data_dict[name] = make_point_positions_from_q(
            robot_output["q"], plant, context, frame, pt
        )
    ...
    plotting_utils.make_plot(
        ...
        dims,
        legend_entries,
        {"title": "Point Positions", "xlabel": "time (s)", "ylabel": "pos (m)"},
        ps,
        subplot_index=subplot_index,
    )
...
def plot_points_velocities(
    ...
    for name in frame_names:
        frame = plant.GetBodyByName(name).body_frame()
        pt = pts[name]
        data_dict[name] = make_point_velocities_from_qv(
            robot_output["q"], robot_output["v"], plant, context, frame, np.zeros(3)
        )
        legend_entries[name] = [name + dim_map[dim] for dim in dims[name]]
    ...
    plotting_utils.make_plot(
        ...
        dims,
        legend_entries,
        {"title": "Point Velocities", "xlabel": "time (s)", "ylabel": "pos (m)"},
        ps,
        subplot_index=subplot_index,
    )
...
def plot_floating_base_body_frame_velocities(
    robot_output, time_slice, plant, context, fb_frame_name
):
    data_dict = {"t": robot_output["t"]}
    data_dict["base_vel"] = get_floating_base_velocity_in_body_frame(
        robot_output, plant, context, plant.GetBodyByName(fb_frame_name).body_frame()
    )[:, 0]
...
def plot_tracking_costs(osc_debug, time_slice):
    ps = plot_styler.PlotStyler()
    data_dict = {key: val for key, val in osc_debug["tracking_cost"].items()}
    ...
    plotting_utils.make_plot(
        ...
        {"xlabel": "Time", "ylabel": "Cost", "title": "tracking_costs"},
        ps,
    )
    return ps
...
def plot_osc_tracking_data_in_space(osc_debug, traj, dims, deriv, time_slice):
    ...
    for dim in dims:
        if deriv == "pos":
            ...
            data["error_ydot_" + str(dim)] = (
                tracking_data.ydot_des[:, dim] - tracking_data.ydot[:, dim]
            )
            ...
            data["yddot_command_sol_" + str(dim)] = tracking_data.yddot_command_sol[
                :, dim
            ]
...
def plot_qp_costs(osc_debug, time_slice):
    ...
    data_dict = {key: val for key, val in regularization_cost.items()}
    data_dict["t_osc"] = osc_debug["t_osc"]
    ps = plot_styler.PlotStyler()
    plotting_utils.make_plot(
        data_dict,
        ...
        {"xlabel": "Time", "ylabel": "Cost", "title": "regularization_costs"},
        ps,
    )
    return ps
...
def plot_qp_solve_time(osc_debug, time_slice):
    ...
    plotting_utils.make_plot(
        ...
        {},
        {},
        {"xlabel": "Timestamp", "ylabel": "Solve Time ", "title": "OSC QP Solve Time"},
        ps,
    )
    return ps
...
def plot_lambda_c_sol(osc_debug, time_slice, lambda_slice):
    ...
    plotting_utils.make_plot(
        ...
        {"xlabel": "time", "ylabel": "lambda", "title": "OSC contact force solution"},
        ps,
    )
    return ps
...
def plot_lambda_h_sol(osc_debug, time_slice, lambda_slice):
    ...
    plotting_utils.make_plot(
        ...
        },
        {
            "xlabel": "time",
            "ylabel": "lambda",
            "title": "OSC constraint force solution",
        },
        ps,
    )
    return ps
    ...
    for i in np.unique(fsm_signal):
        ...
        if fsm_state_names:
            legend_elements.append(
                Patch(facecolor=ps.cmap(2 * i), alpha=0.3, label=fsm_state_names[i])
            )
...
def plot_active_tracking_datas(
    osc_debug, time_slice, fsm_time, fsm_signal, fsm_state_names
):
    ps = plot_styler.PlotStyler()
    ...
    tracking_data_name_dict = {
        "pelvis_trans_traj": "Pelvis Position",
    ...
    for i, tracking_data_name in enumerate(tracking_data_names):
        ...
        tracking_data = osc_debug["osc_debug_tracking_datas"][tracking_data_name]
        ax.fill_between(
            fsm_time,
            ymax - (i + 0.25) / n_tracking_datas,
            ymax - (i + 0.75) / n_tracking_datas,
            where=tracking_data.is_active.astype(bool)[: fsm_time.shape[0]],
            color=ps.cmap(2 * i),
            alpha=0.7,
        )
        tracking_data_legend_elements.append(
            Patch(
                facecolor=ps.cmap(2 * i),
                alpha=0.7,
                label=tracking_data_name_dict[tracking_data_name],
            )
        )
...
def generate_joint_limits(plant):
    joint_position_limits_lower = plant.GetPositionLowerLimits()
    joint_position_limits_upper = plant.GetPositionUpperLimits()
    joint_velocity_limits_lower = plant.GetVelocityLowerLimits()
    joint_velocity_limits_upper = plant.GetVelocityUpperLimits()
    joint_actuator_limits_lower = plant.GetEffortLowerLimits()
    joint_actuator_limits_upper = plant.GetEffortUpperLimits()

    trifinger_joint_position_limit_range = [
        np.min(joint_position_limits_lower),
        np.max(joint_position_limits_upper),
    ]
    trifinger_joint_velocity_limit_range = [
        np.min(joint_velocity_limits_lower),
        np.max(joint_velocity_limits_upper),
    ]
    trifinger_joint_actuator_limit_range = [
        np.min(joint_actuator_limits_lower),
        np.max(joint_actuator_limits_upper),
    ]
    return (
        trifinger_joint_position_limit_range,
        trifinger_joint_velocity_limit_range,
